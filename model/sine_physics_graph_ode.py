"""
Sine Physics-Informed Graph Neural ODE (SinePhyGODE) for climate dynamics

This module implements a physics-informed Graph Neural ODE that combines
the structured approach of PhysicsInformedODE with sine-based Graph Neural Networks
inspired by Kuramoto oscillator dynamics.

Key features:
1. Seasonal linear operator with learnable Fourier coefficients (like XRO)
2. Sine-based nonlinear terms using Kuramoto-inspired message passing
3. ENSO-specific physics constraints with sine aggregation
...
from .physics_graph_ode import SeasonalLinearGraphOperator
...
class SineGraphNonlinearTerms(nn.Module):
    """
    Sine-based nonlinear terms using Kuramoto-inspired message passing
    Combines general nonlinear terms with ENSO-specific physics
    """
    
    def __init__(self, state_dim: int, hidden_dim: int = 64, sine_type: str = 'sine'):
        super().__init__()
        ...
        # Quadratic terms network (x²) with sine aggregation
        if sine_type == 'sine':
            self.quadratic_sine = SineGraphConvBlock(
                input_dim=state_dim,
            ...
            self.quadratic_sine = KuramotoInspiredLayer(
                input_dim=state_dim,
                output_dim=state_dim,
                hidden_dim=hidden_dim
            )
        
        # Cubic terms network (x³) with sine aggregation
        if sine_type == 'sine':
            self.cubic_sine = SineGraphConvBlock(
                input_dim=state_dim,
                hidden_dim=hidden_dim,
                output_dim=state_dim,
                num_layers=2
            ...
            self.cubic_sine = KuramotoInspiredLayer(
                input_dim=state_dim,
                output_dim=state_dim,
                hidden_dim=hidden_dim
            )
        ...
        if state_dim >= 2:
            ...
            if sine_type == 'sine':
                self.enso_T_sine = SineGraphConvBlock(
                ...
                self.enso_T_sine = KuramotoInspiredLayer(
                    input_dim=5,
                    output_dim=1,
                    hidden_dim=32
                )
            ...
            if sine_type == 'sine':
                self.enso_H_sine = SineGraphConvBlock(
                ...
                self.enso_H_sine = KuramotoInspiredLayer(
                    input_dim=5,
                    output_dim=1,
                    hidden_dim=32
                )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Compute nonlinear terms N(x) using sine-based graph networks
        ...
        for b in range(batch_size):
            # Single batch element: [state_dim, state_dim] for graph input
            x_single = x[b:b+1].expand(self.state_dim, -1)  # [state_dim, state_dim]
            
            # Quadratic terms with sine aggregation
            quad_single = self.quadratic_sine(x_single, self.edge_index)  # [state_dim, state_dim]
            quad_single = quad_single.mean(dim=1)  # [state_dim] - average across features
            batch_quad_terms.append(quad_single)
            
            # Cubic terms with sine aggregation
            cubic_single = self.cubic_sine(x_single, self.edge_index)  # [state_dim, state_dim]
        ...
        if self.state_dim >= 2:
            ...
            # Process ENSO terms for each batch element with sine aggregation
            ...
            for b in range(batch_size):
                ...
                enso_T_single = self.enso_T_sine(enso_T_input, self.enso_edge_index)  # [5, 1]
                ...
                enso_H_single = self.enso_H_sine(enso_H_input, self.enso_edge_index)  # [5, 1]
...
class SineGraphNoiseModel(nn.Module):
    """
    Sine-based noise model with seasonal and state dependence
    """
    
    def __init__(self, state_dim: int, sine_type: str = 'sine'):
        super().__init__()
        self.state_dim = state_dim
        ...
        # State-dependent noise scaling using sine aggregation
        self.register_buffer('edge_index', self._create_fully_connected_graph(state_dim + 1))
        
        if sine_type == 'sine':
            self.noise_sine = SineGraphConvBlock(
            ...
            self.noise_sine = KuramotoInspiredLayer(
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        """
        Generate noise scaling using sine aggregation
        ...
        for b in range(batch_size):
            ...
            # State-dependent scaling with sine aggregation
            state_scaling_single = self.noise_sine(graph_input_expanded, self.edge_index)  # [state_dim + 1, state_dim]
...
class SinePhysicsGraphNeuralODEFunc(nn.Module):
    """
    Sine Physics-informed Graph Neural ODE function
    
    dx/dt = L(t) * x + N_sine_graph(x) + ε(t)
    ...
    def __init__(self, state_dim: int, var_names: List[str], hidden_dim: int = 64,
                 sine_type: str = 'sine', fourier_modes: int = 2):
        super().__init__()
        self.state_dim = state_dim
        ...
        # Seasonal linear operator (same as PhysicsGraphNeuralODE)
        self.linear_operator = SeasonalLinearGraphOperator(state_dim, fourier_modes)
        
        # Sine-based nonlinear terms
        self.nonlinear_terms = SineGraphNonlinearTerms(state_dim, hidden_dim, sine_type)
        
        # Sine-based noise model
        self.noise_model = SineGraphNoiseModel(state_dim, sine_type)
        
    def forward(self, t: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        """
        Compute dx/dt using sine physics-informed graph networks
        ...
        # Nonlinear dynamics: N_sine_graph(x)
...
class StochasticSinePhysicsGraphNeuralODEFunc(SinePhysicsGraphNeuralODEFunc):
    """
    Sine Physics-informed Graph Neural ODE function with internal noise
    """
    
    def __init__(self, state_dim: int, var_names: List[str], hidden_dim: int = 64,
                 sine_type: str = 'sine', fourier_modes: int = 2, noise_scale: float = 0.1):
        super().__init__(state_dim, var_names, hidden_dim, sine_type, fourier_modes)
...
class SinePhysicsGraphNeuralODE(nn.Module):
    """
    Sine Physics-informed Graph Neural ODE for climate dynamics
    Combines structured physics (like PhysicsInformedODE) with Sine Graph Neural Networks
    """
    ...
    def __init__(self, state_dim: int, var_names: List[str], hidden_dim: int = 64,
        ...
        if noise_mode == 'internal':
            self.ode_func = StochasticSinePhysicsGraphNeuralODEFunc(
                state_dim, var_names, hidden_dim, sine_type, fourier_modes, noise_scale
            )
        else:
            self.ode_func = SinePhysicsGraphNeuralODEFunc(
                state_dim, var_names, hidden_dim, sine_type, fourier_modes
            )
    ...
    def forward(self, x0: torch.Tensor, t: torch.Tensor,
        ...
        """Forward pass through Sine Physics Graph Neural ODE"""
