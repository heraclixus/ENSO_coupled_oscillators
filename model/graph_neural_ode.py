"""
Graph Neural ODE (GODE) for climate dynamics

This module implements Graph Neural ODEs that use Graph Convolutional Networks (GCN)
or Graph Attention Networks (GAT) as the main neural network components.

Key features:
1. Fully-connected graph structure representing climate variable interactions
2. GCN or GAT layers for learning graph-based dynamics
...
class SeasonalGraphEmbedding(nn.Module):
    """Seasonal embedding for graph-based models"""
...
class GraphConvBlock(nn.Module):
    """Graph convolution block with GCN or GAT layers"""
    
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, 
                 gnn_type: str = 'gcn', num_layers: int = 2, dropout: float = 0.1):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        ...
        if len(self.convs) > len(self.norms):
...
class GraphNeuralODEFunc(nn.Module):
    """
    Graph Neural ODE function using GNN layers
    
    dx/dt = GNN(x, A, s(t)) + ε(t)
    
    Where:
    - GNN: Graph Neural Network (GCN or GAT)
    ...
    def __init__(self, state_dim: int, var_names: List[str], hidden_dim: int = 64,
        ...
        # Main GNN dynamics
        self.gnn_dynamics = GraphConvBlock(
    ...
    def forward(self, t: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        """
        Compute dx/dt using Graph Neural Networks
        ...
        # Process each batch element separately (simpler approach)
        batch_outputs = []
        for b in range(batch_size):
            ...
            # Apply GNN dynamics
            x_gnn_single = self.gnn_dynamics(x_single, self.edge_index)  # [state_dim, hidden_dim]
            
            # Output projection
            dxdt_single = self.output_projection(x_gnn_single)  # [state_dim, state_dim]
...
class StochasticGraphNeuralODEFunc(GraphNeuralODEFunc):
    """
    Graph Neural ODE function with noise directly in the vector field (internal noise)
    """
    
    def __init__(self, state_dim: int, var_names: List[str], hidden_dim: int = 64,
                 gnn_type: str = 'gcn', num_gnn_layers: int = 2, noise_scale: float = 0.1):
        super().__init__(state_dim, var_names, hidden_dim, gnn_type, num_gnn_layers)
...
class GraphNeuralODE(nn.Module):
    """
    Graph Neural ODE for climate dynamics
    ...
    def __init__(self, state_dim: int, var_names: List[str], hidden_dim: int = 64,
        ...
        if noise_mode == 'internal':
            self.ode_func = StochasticGraphNeuralODEFunc(
                state_dim, var_names, hidden_dim, gnn_type, num_gnn_layers, noise_scale
            )
        else:
            self.ode_func = GraphNeuralODEFunc(
                state_dim, var_names, hidden_dim, gnn_type, num_gnn_layers
            )
    ...
    def forward(self, x0: torch.Tensor, t: torch.Tensor, 
        ...
        """
        Forward pass through Graph Neural ODE
    ...
    def simulate(self, x0_data: xr.Dataset, nyear: int = 5, ncopy: int = 1, 
        ...
        """
        Generate simulations using the Graph Neural ODE
    ...
    def reforecast(self, init_data: xr.Dataset, n_month: int = 21, ncopy: int = 1,
        ...
        """
        Generate reforecasts using the Graph Neural ODE
